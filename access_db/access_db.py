...
def get_clean_list (list:list)->list:
	return [get_clean_str(row) for row in list]

def get_clean_str(str:str)->str:
...
def get_max_ndim(geom_series)->int:
	max_ndim = 2
...
def get_ndim(geom)->int:
	try:
		return coalesce(geom._ndim,0)
	except:
		return 0

def st_force3d_astxt(geom)->str:
	# print(dir(geom))
	#		 
	if geom.has_z:
		return geom.wkt
...
def st_force3d_asewkt(geom,srid:str)->str:
	if str(geom) == 'None':
		return 'null'
...
def st_asewkt(geom,srid:str)->str:
	if str(geom) == 'None':
		return 'null'
...
def get_engine_str_with_env(db_env:str, use_cloud_sql_name:Union[bool,None]=None, force_print:bool=False )->str:
	db_env = db_env.upper()
...
def get_engine_str_with_json(db_json:dict, use_cloud_sql_name:Union[bool,None]=None, force_print:bool=False )->str:
...
def get_df_sql(sql_query_string:str, engrawconn_db=None , db_json:Union[dict,None]=None, db_env:Union[str,None]=None , db_secret:Union[dict,None]=None, return_type:str='df' , geom_cols:list = ['geometry'] , force_print:bool=False)->Union[pd.DataFrame, gpd.GeoDataFrame]:
...
def close_rawconn_and_disp_eng(engrawconn_db, engrawconn_db_none:bool = True)->None:
	if engrawconn_db_none:
		engine_db, rawconn_db = engrawconn_db
		rawconn_db.close()
		engine_db.dispose()

def check_table_exists(table:str, schema:str, engrawconn_db=None , db_json:Union[dict,None]=None, db_env:Union[str,None]=None , db_secret:Union[dict,None]=None )->bool:
