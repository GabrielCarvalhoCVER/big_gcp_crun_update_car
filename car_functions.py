...
def download_car(*args):
    ...
    if len(args) == 1 and (isinstance(args[0], gpd.geodataframe.GeoDataFrame) or isinstance(args[0], pd.DataFrame)):
        df = args[0]
        n_row = len(df)
        df['Download State'] =''
        ...
        elif (settings.CITY_COLUMN in args[0].columns):
            print('city_column  :' + settings.CITY_COLUMN)
            city_list = args[0].loc[:, [settings.CITY_COLUMN, settings.STATE_COLUMN]]
